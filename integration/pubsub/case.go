...
const (
	caseName = "ai.vanus.integration.pubsub"
)
...
func Test(ctx context.Context, endpoint string, num int64, timeout time.Duration) (err error) {
	log.Info().Str("endpoint", endpoint).Msgf("Testing vanus: %s", caseName)
	...
	// Create an eventbus.
	ebName := fmt.Sprintf("regression-pubsub-%s", rand.RandString(5))
	...
	req := &ctrlpb.SubscriptionRequest{
		...
		Name:        ebName,
		Description: "regression test: pubsub",
		EventbusId:  eb.Id,
	...
	generator := event.NewSimpleGenerator(constant.TestRepo, caseName, maximumPayloadSize)
	if err = event.Publish(ctx, eb.Id, num, parallelism, generator, client, tracer, &publishStats); err != nil {
		cancel()
		return err
	}

	success := tracer.Wait(timeout)
